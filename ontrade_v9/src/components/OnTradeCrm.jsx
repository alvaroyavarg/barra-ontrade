...
import { AACC_AGREEMENTS } from "../data/aaccMockData.js";

const SCORE_OK = new Set(["Completado", "Bueno", "Fuerte"]);
const isGoodScore = (s) => SCORE_OK.has(s);

const PILLAR_KEYS   = ["staff", "assortment", "menu", "branding", "activation"];
const PILLAR_LABELS = { staff: "Staff", assortment: "Assortment", menu: "Menú", branding: "Branding", activation: "Activación" };

function pillarPct(locals, key) {
  const relevant = locals.filter((l) => l.pillars?.[key]?.score && l.pillars[key].score !== "Sin registro");
  if (relevant.length === 0) return 0;
  const good = relevant.filter((l) => isGoodScore(l.pillars[key].score)).length;
  return Math.round((good / relevant.length) * 100);
}

function onFiveScoreFor(locals) {
  if (locals.length === 0) return 0;
  return Math.round(PILLAR_KEYS.reduce((sum, k) => sum + pillarPct(locals, k), 0) / PILLAR_KEYS.length);
}
...
function scoreBadgeClass(pct) {
  if (pct >= 70) return "crm-badge--green";
  if (pct >= 40) return "crm-badge--amber";
  return "crm-badge--red";
}

const SKU_TO_BRAND = {
  JW: "Johnnie Walker", SMIR: "Smirnoff", DJ: "Don Julio", TQ: "Tanqueray",
  BAILEYS: "Baileys", CIROC: "Cîroc", ZACAPA: "Zacapa", BULLEIT: "Bulleit",
  BUCHANAN: "Buchanan's", SINGLETON: "The Singleton", GORDONS: "Gordon's", OLDPARR: "Old Parr",
};

function skuToBrand(sku) {
  const u = sku.trim().toUpperCase();
  if (u.startsWith("JW")) return "Johnnie Walker";
  if (u.startsWith("SMIR")) return "Smirnoff";
  if (u.startsWith("DJ")) return "Don Julio";
  if (u.startsWith("TQ")) return "Tanqueray";
  for (const [prefix, brand] of Object.entries(SKU_TO_BRAND)) {
    if (u.startsWith(prefix)) return brand;
  }
  return null;
}

function brandPresence(locals) {
  const brands = {};
  for (const local of locals) {
    if (!local.skus) continue;
    const seen = new Set();
    for (const sku of local.skus.split(",")) {
      const brand = skuToBrand(sku);
  ...
  return Object.entries(brands)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 6)
    .map(([brand, count]) => ({ brand, count, pct: Math.round((count / locals.length) * 100) }));
}

function aaccDecisionStatus(agreement) {
  const seg = agreement.segmentation ?? "";
  const dur = agreement.durationMonths ?? 12;
  const inv = agreement.totalInvestmentUsd ?? 0;
  if (seg.includes("TOP PREMIUM")) return "boost";
  if (dur >= 36 && inv > 20000) return "renegotiate";
  if (dur >= 36) return "maintain";
  if (inv > 8000) return "maintain";
  return "freeze";
}

const DECISION_CONFIG = {
  boost:       { label: "Potenciar",    color: "#15803D", bg: "#DCFCE7", badge: "crm-badge--green" },
  maintain:    { label: "Mantener",     color: "#1D4ED8", bg: "#DBEAFE", badge: "crm-badge--blue" },
  renegotiate: { label: "Renegociar",   color: "#92400E", bg: "#FEF9C3", badge: "crm-badge--amber" },
  freeze:      { label: "Congelar",     color: "#B91C1C", bg: "#FEE2E2", badge: "crm-badge--red" },
  exit:        { label: "No Renovar",   color: "#6B7280", bg: "#F3F4F6", badge: "crm-badge--gray" },
};
...
function PillarDot({ score }) {
  let bg = "#D1D5DB";
  if (score === "Atencion") bg = "#EF4444";
  else if (score === "Pendiente") bg = "#F59E0B";
  else if (isGoodScore(score)) bg = "#22C55E";
  return <span style={{ width: 9, height: 9, borderRadius: "50%", background: bg, display: "inline-block", flexShrink: 0 }} title={score} />;
}
...
function WalkerDashboard({ columns, locals, summary, excelMeta, excelError, draggedCardId, onCardDragStart, onCardDrop, onOpenLocal, onUpload }) {
  ...
  const onFiveScore  = onFiveScoreFor(locals);
  ...
  const gapsCriticos = locals.filter((l) => {
    if (!l.pillars) return false;
    const bad = PILLAR_KEYS.filter((k) => l.pillars[k]?.score && !isGoodScore(l.pillars[k].score) && l.pillars[k].score !== "Sin registro");
    return bad.length >= 3;
  }).length;

  const rutaDelDia = [...locals]
    .sort((a, b) => (a.healthScore ?? 99) - (b.healthScore ?? 99))
    .slice(0, 8);

  const topRecuperar = [...locals]
    .sort((a, b) => (a.healthScore ?? 99) - (b.healthScore ?? 99))
    .slice(0, 3);
    ...
    <div className="crm-stack">
      <section className="crm-metrics-grid crm-metrics-grid--four">
        ...
        <KpiCard
          ...
          icon="🚨"
          question="¿Qué urge?"
          label="Gaps críticos (3+ pilares)"
          value={gapsCriticos}
          note={gapsCriticos > 0 ? "cuentas con ejecución deficiente" : "sin alertas críticas"}
        />
      </section>

      <section className="crm-dashboard-grid">
        <article className="crm-card">
          <SectionTitle kicker="Prioridad hoy" title="Ruta del día" description="Cuentas ordenadas por health score — las que más necesitan atención primero." />
          <div className="crm-ruta-list">
            <div className="crm-ruta-row crm-ruta-row--header">
              <span>Cuenta</span>
              <span style={{ textAlign: "center" }}>ST</span>
              <span style={{ textAlign: "center" }}>AS</span>
              <span style={{ textAlign: "center" }}>ME</span>
              <span style={{ textAlign: "center" }}>BR</span>
              <span style={{ textAlign: "center" }}>AC</span>
              <span style={{ textAlign: "right" }}>Score</span>
            </div>
            {rutaDelDia.map((local) => (
              <button key={local.id} type="button" className="crm-ruta-row crm-ruta-row--item" onClick={() => onOpenLocal(local.id)}>
                <span className="crm-ruta-row__name">
                  <strong>{local.name}</strong>
                  <small>{local.district}</small>
                </span>
                {PILLAR_KEYS.map((k) => (
                  <span key={k} style={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
                    <PillarDot score={local.pillars?.[k]?.score ?? "Sin registro"} />
                  </span>
                ))}
                <span style={{ textAlign: "right", fontWeight: 700, fontSize: "0.84rem", color: scoreColor(local.healthScore ?? 0) }}>
                  {local.healthScore ?? "—"}
                </span>
              </button>
            ))}
          </div>
          <div className="crm-ruta-legend">
            {[["#22C55E","Bueno/OK"],["#F59E0B","Pendiente"],["#EF4444","Atención"],["#D1D5DB","Sin registro"]].map(([color, label]) => (
              <span key={label} style={{ display: "flex", alignItems: "center", gap: 4, fontSize: "0.72rem", color: "#6B7280" }}>
        ...
        <div style={{ display: "grid", gap: 10 }}>
          <article className="crm-card">
            ...
            <div className="crm-progress-list" style={{ marginTop: 8 }}>
              {PILLAR_KEYS.map((k) => (
                <ProgressRow key={k} label={PILLAR_LABELS[k]} value={pillarPct(locals, k)} />
              ))}
            </div>
          </article>

          <article className="crm-card">
            <SectionTitle kicker="Acción requerida" title="Top 3 a recuperar" />
            <div style={{ display: "grid", gap: 6, marginTop: 8 }}>
              {topRecuperar.map((local) => {
                const gaps = PILLAR_KEYS.filter((k) => local.pillars?.[k]?.score && !isGoodScore(local.pillars[k].score) && local.pillars[k].score !== "Sin registro");
                return (
                  <button
                    key={local.id}
                    type="button"
                    onClick={() => onOpenLocal(local.id)}
                    className="crm-recover-row"
                  >
                    <div>
                      <strong>{local.name}</strong>
                      <small>{local.subchannel} · {local.district}</small>
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      {gaps.length > 0 && (
                        <span className="crm-badge crm-badge--red">{gaps.length} gap{gaps.length > 1 ? "s" : ""}</span>
                      )}
                      <span style={{ fontWeight: 700, fontSize: "0.88rem", color: scoreColor(local.healthScore ?? 0) }}>
                        {local.healthScore ?? "—"}
                      </span>
                    </div>
                  </button>
...
function CpaDashboard({ locals = [], walkers = [] }) {
  ...
  const teamOnFive    = onFiveScoreFor(locals);

  const totalInvUSD   = AACC_AGREEMENTS.reduce((s, a) => s + (a.totalInvestmentUsd ?? 0), 0);
  const avgInvUSD     = AACC_AGREEMENTS.length > 0 ? Math.round(totalInvUSD / AACC_AGREEMENTS.length) : 0;

  const decisionGroups = useMemo(() => {
    const groups = { boost: [], maintain: [], renegotiate: [], freeze: [], exit: [] };
    for (const ag of AACC_AGREEMENTS) {
      const key = aaccDecisionStatus(ag);
      groups[key].push(ag);
    }
    return groups;
  }, []);

  const pilarScores = PILLAR_KEYS.map((k) => ({ key: k, label: PILLAR_LABELS[k], pct: pillarPct(locals, k) }));

  const brands = useMemo(() => brandPresence(locals), [locals]);

  const MONTH_LABELS = ["Jul", "Ago", "Sep", "Oct", "Nov", "Dic", "Ene", "Feb", "Mar", "Abr"];
  const mockTrend = [62, 71, 68, 80, 75, 92, 85, 88, 94, 97];
    ...
    <div className="crm-stack">
      <section className="crm-metrics-grid crm-metrics-grid--four">
        <KpiCard
          color="blue"
          icon="💰"
          question="¿Cuánto invertimos?"
          label="Inversión AACC comprometida"
          value={`USD ${(totalInvUSD / 1000).toFixed(0)}K`}
          note={`${AACC_AGREEMENTS.length} acuerdos · Prom. USD ${(avgInvUSD / 1000).toFixed(1)}K`}
        />
        <KpiCard
          color="green"
          icon="📊"
        ...
        <KpiCard
          color="purple"
          icon="⚡"
          question="¿Cómo ejecuta el canal?"
          label="On Five score promedio"
          value={`${teamOnFive}%`}
          progress={teamOnFive}
          trend={teamOnFive >= 70 ? " ↑ En meta" : " ↓ Bajo meta"}
        />
        <KpiCard
          color="amber"
          icon="👥"
          ...
          note={`${totalLocals} cuentas en portafolio`}
        />
      </section>

      <section className="crm-dashboard-grid">
        <article className="crm-card">
          <SectionTitle kicker="Portfolio AACC" title="Decision Board" description="Estado de los acuerdos comerciales según desempeño y segmento." />
          <div className="crm-decision-board">
            {Object.entries(DECISION_CONFIG).map(([key, cfg]) => {
              const items = decisionGroups[key] ?? [];
              const inv   = items.reduce((s, a) => s + (a.totalInvestmentUsd ?? 0), 0);
              return (
                <div key={key} className="crm-decision-item" style={{ background: cfg.bg, borderColor: cfg.color + "40" }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                    <span style={{ fontWeight: 700, fontSize: "0.8rem", color: cfg.color }}>{cfg.label}</span>
                    <span style={{ fontWeight: 800, fontSize: "1.4rem", color: cfg.color, lineHeight: 1 }}>{items.length}</span>
                  </div>
                  <div style={{ fontSize: "0.74rem", color: cfg.color, opacity: 0.8 }}>
                    {inv > 0 ? `USD ${(inv / 1000).toFixed(0)}K inversión` : "Sin inversión asignada"}
                  </div>
                  {items.length > 0 && (
                    <div style={{ marginTop: 8, display: "grid", gap: 3 }}>
                      {items.slice(0, 2).map((a) => (
                        <div key={a.agreementId} style={{ fontSize: "0.73rem", color: cfg.color, opacity: 0.9, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                          {a.customerName}
                        </div>
                      ))}
                      {items.length > 2 && <div style={{ fontSize: "0.72rem", color: cfg.color, opacity: 0.6 }}>+{items.length - 2} más</div>}
                    </div>
                  )}
                </div>
              );
        ...
        <article className="crm-card">
          <SectionTitle kicker="Ejecución PDV" title="On Five por pilar" />
          <div className="crm-progress-list" style={{ marginTop: 8 }}>
      ...
      <section className="crm-dashboard-grid">
        <article className="crm-card">
          <SectionTitle kicker="Portafolio PDV" title="Mix de marcas en ejecución" description="% de cuentas donde cada marca tiene presencia declarada en SKU." />
          <div style={{ display: "grid", gap: 10, marginTop: 8 }}>
            {brands.map(({ brand, count, pct }) => (
              <div key={brand} style={{ display: "grid", gridTemplateColumns: "130px 1fr 50px 40px", alignItems: "center", gap: 10 }}>
                <span style={{ fontSize: "0.82rem", fontWeight: 600, color: "#374151" }}>{brand}</span>
                <div className="crm-bar">
                  <div className="crm-bar__fill" style={{ width: `${pct}%` }} />
                </div>
                <span style={{ fontSize: "0.78rem", color: "#6B7280", textAlign: "right" }}>{pct}%</span>
                <span style={{ fontSize: "0.72rem", color: "#9CA3AF" }}>{count}</span>
              </div>
            ))}
            {brands.length === 0 && (
              <p style={{ color: "#9CA3AF", fontSize: "0.84rem" }}>Sin datos de SKU disponibles.</p>
            )}
          </div>
        </article>

        <article className="crm-card">
          <SectionTitle kicker="Tendencia" title="Ejecución mensual" description="On Five score promedio del canal por mes. Los datos de venta se conectarán en siguiente fase." />
          <div className="crm-trend-chart">
            <div className="crm-trend-bars">
              {mockTrend.map((val, i) => (
                <div key={i} className="crm-trend-col">
                  <div className="crm-trend-bar" style={{ height: `${val}%` }} title={`${val}%`} />
                  <span className="crm-trend-label">{MONTH_LABELS[i]}</span>
                </div>
              ))}
            </div>
            <div className="crm-trend-axis">
              {[100, 75, 50, 25].map((v) => (
                <span key={v} style={{ fontSize: "0.66rem", color: "#9CA3AF" }}>{v}%</span>
              ))}
            </div>
          </div>
          <p style={{ marginTop: 8, fontSize: "0.74rem", color: "#9CA3AF", fontStyle: "italic" }}>
            Datos ilustrativos — se conectará con ventas reales en siguiente fase.
          </p>
        </article>
      </section>
    </div>
...
function ManagerDashboard({ locals = [], walkers = [] }) {
  ...
  const walkerRows = walkers.map((w) => {
    const wLocals   = locals.filter((l) => l.walkerName === w.name || l.walker === w.id);
    const audit     = wLocals.filter((l) => l.pillars && Object.values(l.pillars).some((p) => p.lastAudit)).length;
    const cobertura = wLocals.length > 0 ? Math.round((audit / wLocals.length) * 100) : 0;
    const pilarPcts = PILLAR_KEYS.map((k) => pillarPct(wLocals, k));
    const score     = pilarPcts.length > 0 ? Math.round(pilarPcts.reduce((a, b) => a + b, 0) / pilarPcts.length) : 0;
    return { ...w, total: wLocals.length, auditados: audit, cobertura, pilarPcts, score };
  });

  const alertas = [...locals]
    .sort((a, b) => (a.healthScore ?? 99) - (b.healthScore ?? 99))
    .slice(0, 5);

  const aaccTotal     = AACC_AGREEMENTS.length;
  const aaccKOA       = AACC_AGREEMENTS.filter((a) => a.bottler === "KOA").length;
  const aaccKOE       = AACC_AGREEMENTS.filter((a) => a.bottler === "KOE").length;
  const totalInvUSD   = AACC_AGREEMENTS.reduce((s, a) => s + (a.totalInvestmentUsd ?? 0), 0);
  const topPremium    = AACC_AGREEMENTS.filter((a) => (a.segmentation ?? "").includes("TOP PREMIUM")).length;
    ...
    <div className="crm-stack">
      <section className="crm-metrics-grid crm-metrics-grid--four">
        ...
        <KpiCard
          ...
          label="On Five score del equipo"
          value={`${teamOnFive}%`}
          progress={teamOnFive}
          trend={teamOnFive >= 70 ? " ↑ En meta" : " ↓ Bajo meta"}
        />
        <KpiCard
          color="amber"
          icon="💰"
          question="¿Qué comercial?"
          label="AACCs activos en portafolio"
          value={aaccTotal}
          note={`KOA: ${aaccKOA} · KOE: ${aaccKOE}`}
        />
      </section>

      <section className="crm-dashboard-grid">
        <article className="crm-card">
          <SectionTitle kicker="Equipo Walker" title="Performance por pilar" description="% cuentas con ejecución OK por cada pilar de On Five." />
          <div className="crm-heatmap">
            <div className="crm-heatmap__header">
              <span>Walker</span>
              {PILLAR_KEYS.map((k) => <span key={k}>{PILLAR_LABELS[k]}</span>)}
              <span>Score</span>
            </div>
            {walkerRows.map((w) => (
              <div key={w.id} className="crm-heatmap__row">
                <div className="crm-heatmap__name">
                  <strong>{w.name}</strong>
                  <small>{w.total} ctas · {w.cobertura}% cob.</small>
                </div>
                {w.pilarPcts.map((pct, i) => (
                  <div key={i} className="crm-heatmap__cell">
                    <span style={{ color: scoreColor(pct), fontWeight: 700, fontSize: "0.82rem" }}>{pct}%</span>
                    <div className="crm-bar" style={{ height: 4, marginTop: 3 }}>
                      <div className="crm-bar__fill" style={{ width: `${pct}%`, background: scoreColor(pct) }} />
                    </div>
                  </div>
                ))}
                <div className="crm-heatmap__cell">
                  <span className={`crm-badge ${scoreBadgeClass(w.score)}`}>{w.score}%</span>
                </div>
              </div>
            ))}
            {walkerRows.length === 0 && (
              <p style={{ color: "#9CA3AF", fontSize: "0.84rem", padding: "16px", textAlign: "center" }}>
                Sin datos de walkers disponibles.
              </p>
            )}
          </div>
        </article>
        ...
        <div style={{ display: "grid", gap: 10 }}>
          <article className="crm-card">
            <SectionTitle kicker="Portfolio AACC" title="Acuerdos comerciales" />
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 10 }}>
              <MetricCard label="Total acuerdos" value={aaccTotal} />
              <MetricCard label="Inversión total" value={`USD ${(totalInvUSD / 1000).toFixed(0)}K`} />
              <MetricCard label="Top Premium" value={topPremium} note="acuerdos segmento alto" />
              <MetricCard label="Inv. promedio" value={`USD ${Math.round(totalInvUSD / (aaccTotal || 1) / 100) * 100}`} note="por acuerdo" />
            </div>
          </article>

          <article className="crm-card">
            <SectionTitle kicker="Alertas" title="Cuentas a revisar" />
            <div style={{ display: "grid", gap: 6, marginTop: 8 }}>
              {alertas.map((local) => {
                const gaps = PILLAR_KEYS.filter((k) => local.pillars?.[k]?.score && !isGoodScore(local.pillars[k].score) && local.pillars[k].score !== "Sin registro");
                return (
                  <div key={local.id} className="crm-alert-row">
                    <div>
                      <strong>{local.name}</strong>
                      <small>{local.walkerName} · {local.district}</small>
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      {gaps.length > 0 && <span className="crm-badge crm-badge--amber">{gaps.length} gap{gaps.length > 1 ? "s" : ""}</span>}
                      <span style={{ fontWeight: 700, fontSize: "0.82rem", color: scoreColor(local.healthScore ?? 0) }}>{local.healthScore ?? "—"}</span>
                    </div>
                  </div>
                );
              })}
